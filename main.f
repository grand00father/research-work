...
*                                                                      *
* Ðàñ÷¸ò ÿâíûì ìåòîäîì ÌàêÊîðìàêà óðàâíåíèÿ kdvb ñ ó÷¸òîì "ïåëåíû"     *
*    øèðèíîé [x1, x2], x2=x1+hx                                        *
* Ñêâîçíîé ñ÷¸ò.                                                       *
* Ìåòîä êîððåêöèè ïîòîêîâ (ìåòîä FCT).                                 *
*                                                                      *
*     Intrinsic function: MAX, MIN, DEXP, DSQRT, IDSIGN, IDINT         *
*                                                                      *
*     Îáîçíà÷åíèÿ:                                                     *
* cNV - number of variant                                              *
* i_shape=0 - ïðÿìîóãîëüíèê                                            *
*   h_max - âûñîòà, [-jx, jx] - øèðèíà (êîë-âî óçëîâ)                  *                                 *
* i_shape=1 - ñîëèòîí		                                               *
* i_shape=2 - ñòóïåíüêà	                                               *
* [x1, x1+hx] - øèðèíà "ïåëåíû", x1 - íà÷àëî (x1>j0), hx - øèðèíà      *
* ta - step over time                                                  *
* dx - øàã ïî êîîðäèíàòå x                                             *
* t_stop - ìàêñèìàëüíîå âðåìÿ âû÷èñëåíèÿ                               *
* istep - number of steps				                                       *
* i_corr=0 - êîððåêöèÿ ïîòîêîâ                                         *
************************************************************************
...
**********     Íà÷àëüíîå ðàñïðåäåëåíèå      **************
      ...
      SELECT CASE(i_shape)
        ...
        CASE(0)             ! âèä ïðÿìîóãîëüíèêà (i_shape=0)
          DO j=j0-jx,j0+jx
            h(j)=h_max
          END DO

        CASE(1)             ! òèï ñîëèòîíà (i_shape=1)
          IF(bet1.EQ.0) THEN
        ...
        CASE(10)       ! Áåç ðàñïðåäåëåíèÿ
          CONTINUE

      END SELECT

      IF(t_stop.EQ.0.0) THEN       ! âûâîä íà÷àëüíîãî ðàñïðåäåëåíèÿ
        DO j=1,j2
          x=(j-j0)*dx
          WRITE(35,410) x,h(j)
        END DO
...
**     Çàïèñü íà÷àëüíîãî ðàñïðåäåëåíèÿ     ***
...
*     Ïîñòîÿííûå
      ...
      mac=-1     ! ïåðåêëþ÷àòåëü äëÿ ÌàêÊîðìàêà
...
******                                                       ***********
*               Ïåðâûé øàã. Ðàçíîñòè "íàçàä"    mac=-1                       *
******                                                       ***********
...
******                                                       ***********
*                Âòîðîé øàã. Ðàçíîñòè "âïåð¸ä"    mac=-1                     *
******                                                       ***********

        DO j=m1,m2    ! Âû÷èñëåíèå èñòî÷íèêà â ïðîìåæóòî÷íîé òî÷êå
     ...
     4          fj2(j)
        ...
        GO TO 23      ! Îáõîä ñëó÷àÿ mac=+1
      END IF

******                                                       ***********
*               Ïåðâûé øàã. Ðàçíîñòè "âïåð¸ä"    mac=+1                *
******                                                       ***********
...
******                                                       ***********
*                Âòîðîé øàã. Ðàçíîñòè "íàçàä"      mac=+1              *
******                                                       ***********
   ...
   23 CONTINUE     ! Ïåðåõîä ïîñëå ñëó÷àÿ äëÿ ÌàêÊîðìàêà mac=-1

      IF(i_corr.EQ.1) GOTO 27   ! îáõîä êîððåêöèè  =================

****************    ÊÎÐÐÅÊÖÈß ÏÎÒÎÊÎÂ    **********************
*
*       Îáîçíà÷åíèÿ
* ur - ïîëóñóììà ñêîðîñòåé
* cu_d - äèôôóçèîííûå êîýôôèöèåíòû
* cu_ad - àíòèäèôôóçèîííûå êîýôôèöèåíòû
*
************************************************************

*     Âû÷èñëåíèå êîýôôèöèåíòîâ
...
*       1. Âû÷èñëåíèå äèôôóçèîííûõ ïîòîêîâ

        u_d(j)=cu_d(j)*(u0(jr)-u0(j))

*       2. Âû÷èñëåíèå àíòèäèôôóçèîííûõ ïîòîêîâ

        u_ad(j)=cu_ad(j)*(u1(jr)-u1(j))

      END DO

*     3. ×èñëåííàÿ äèôôóçèÿ ðåøåíèÿ

      DO j=2,j9
        u1(j)=u1(j)+u_d(j)-u_d(j-1)
      END DO

*     4. Âû÷èñëåíèå ïåðâûõ ðàçíîñòåé

      DO j=1,j9
        d_u(j)=u1(j+1)-u1(j)
      END DO

*     5. Îãðàíè÷åíèå àíòèäèôôóçèîííûõ ïîòîêîâ.
...
*     6. Îêîí÷àòåëüíîå ðåøåíèå.
   ...
   27 continue    !  Òî÷êà îáõîäà êîððåêöèè ïîòîêîâ

****     âû÷èñëåíèå èíòåãðàëà S   ******
   ...
   70 FORMAT('step=',I8,' time=',f8.2,
     ...
     2       ' u_x1=',d9.2,' u_x2=',d9.2)
      ...
      IF(time.GE.open_time(i_open).AND.i_open.LT.i_open0) THEN
        ...
        DO j=1,j2      ! Çàïèñü ðåçóëüòàòîâ â ôàéë
          x=(j-j0)*dx
          WRITE(35,410) x,u1(j)
        END DO

        IF(time.GE.t_stop) GO TO 80   !  îêîí÷àíèå ñ÷¸òà
